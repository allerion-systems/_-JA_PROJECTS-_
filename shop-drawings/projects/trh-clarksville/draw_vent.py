"""Generate the shop drawing package for the 5V roof vent at TRH Clarksville.
...
SM-1  assembly — plan, section through the slope, throat elevation, panel
      profile, bill of materials, ventilation calculation, general notes.
SM-2  fabrication — flat patterns with bend lines, bend schedule, cut list,
      fastener and sealant schedules, forming notes, open RFI items.
"""
...
def sheet_assembly(s: Sheet):
    ...
    # ------------------------------------------------------------ 1  PLAN
    vp = Viewport(s, 3.30 * IN, 7.15 * IN, 0.100)
    half = G.base_w / 2.0
    y0, y1 = -G.flange_dn, G.opening_l + G.flange_up
    ox, oy = G.opening_w / 2.0, G.opening_l
    ...
    # the vent's own sidelap double-V
    for sgn in (-1, 1):
    ...
    vp.rect(-ox - G.curb_h, -G.curb_h, G.opening_w + 2 * G.curb_h,
            oy + 2 * G.curb_h, lw=LW_THIN, dash=DASH_HIDDEN)
    vp.rect(-ox, 0, G.opening_w, oy, lw=LW_OBJECT)
    vp.rect(-ox - G.hood_lap, -G.hood_overhang,
            G.opening_w + 2 * G.hood_lap, G.hood_l + G.hood_overhang,
            lw=LW_HIDDEN, dash=DASH_HIDDEN)
    vp.centerline(0, y0 - 3, 0, y1 + 3)
    vp.bend_line(-half, 0, half, 0)
    vp.bend_line(-half, oy, half, oy)

    # fastener stations across the module (UL 580 Const. #453)
    ...
    vp.dim_h(-ox, ox, y1, offset=0.40)
    vp.dim_v(y0, y1, half, offset=1.18)
    vp.dim_v(0, oy, -half, offset=-0.46)
    vp.dim_v(oy, y1, half, offset=0.50, text=f'{fmt_in(G.flange_up)} UPSLOPE')
    ...
    vp.leader(ox + G.hood_lap, -G.hood_overhang * 0.6, half + 3.0, -7.5,
              "HOOD ABOVE (DASHED)")
    ...
    vp.section_mark(-half - 2.5, oy * 0.42, half + 2.5, oy * 0.42, "A", 0, -1, "SM-1")
    vp.view_title(-half - 4.0, y0 - 12.5, "PLAN — VENT IN ROOF PLANE",
                  "1-1/5\"=1'-0\"", "1")

    # --------------------------------------------------------- 2  SECTION
    vs = Viewport(s, 2.345 * IN, 2.55 * IN, 0.145)
    ya, yb = -G.flange_dn - 5, G.opening_l + G.flange_up + 5
    ...
    vs.line(ya, -0.15, yb, -0.15, lw=0.6, color=GRAY)
    vs.text(yb - 13.5, -0.27, "GAF STORMGUARD HT LEAK BARRIER", size=4.8, color=GRAY)

    # panel below the vent; ridge above
    vs.line(ya, 0, -G.flange_dn - 0.4, 0, lw=LW_OBJECT)
    # ridge cap over the rear counterflashing
    ...
    vs.dim_h(0, G.opening_l, -2.1, offset=0.0)
    ...
    vs.dim_h(G.opening_l, G.opening_l + G.flange_up, -3.3, offset=0.0)

    vs.balloon(G.opening_l * 0.55, G.curb_h, G.opening_l * 0.40, -5.4, 1)
    vs.balloon(G.hood_l * 0.42, G.hood_rise * 0.78, G.hood_l * 0.25,
               G.hood_rise + 3.9, 2)
    vs.balloon(G.opening_l + 0.35, G.hood_rise * 0.55,
               G.opening_l + 7.0, G.hood_rise + 3.9, 3)
    ...
    vs.multileader(G.opening_l + G.flange_up * 0.55, 0.06,
                   G.opening_l + G.flange_up - 2.0, -6.0,
                   ["UPSLOPE FLANGE RUNS TO THE RIDGE.",
                    "BUTYL TAPE FULL WIDTH.",
                    "NO EXPOSED FASTENERS IN WATERWAY."])
    ...
    vs.text(ya + 0.5, G.hood_rise + 5.2,
    ...
    vn.leader(-G.base_w / 2, PANEL.rib_h, -14.0, 4.2, "DOUBLE V AT SIDELAP")
    ...
    vd = Viewport(s, 4.555 * IN, 8.35 * IN, 0.22)
    ry = G.ridge_y
    ...
    # pan: upslope curb + flange to the ridge
    vd.metal_section([(da, G.curb_h), (G.opening_l, G.curb_h),
                      (G.opening_l, 0.0), (ry, 0.0)], thk_pts=2.6)
    # hood back plate + top
    vd.metal_section([(da, G.hood_rise - (G.hood_rise - G.throat_h)
                       * (G.hood_l - da) / G.hood_l),
                      (G.hood_l, G.hood_rise), (G.hood_l, 0.10)], thk_pts=2.6)
    # rear counterflashing
    vd.metal_section(G.section_rear_cf(), thk_pts=2.3)
    # ridge cap
    vd.polyline([(ry - 2.6, 0.62), (ry + 0.35, 0.62), (ry + 0.35, 2.2)],
                lw=1.7)
    ...
    vd.dim_v(0, G.curb_h, G.opening_l - 1.4, offset=0.0,
             text=f'{fmt_in(G.curb_h)} CURB')

    vd.leader(G.hood_l + 1.3, 0.16, G.hood_l - 1.0, -2.6,
    ...
    y = table(s, rx, y, [20, 172, 96, 28, 38],
              [["1", "Base pan / deck flashing, 5V edges", "24 ga Galvalume", "1", "SM-2"],
               ["2", "Hood — throat face + top", "24 ga Galvalume", "1", "SM-2"],
              ...
              title="BILL OF MATERIALS — ONE (1) VENT ASSEMBLY",
              align=["c", "l", "l", "c", "c"])
    ...
    y = table(s, rx, y, [196, 158],
              [["Gross throat opening",
               ...
               ["Screen / blockage derate", f"{G.screen_free_pct:.2f} (estimate)"],
               ["NET FREE AREA per vent", f"{G.net_free_area:.0f} sq in — CALCULATED"],
               ["Code basis", "IBC 1202.2.1 / IRC R806.2"],
               ["Required", "1/150 of vented area; 1/300 if 40-50%"],
               ["", "of the area is in the upper portion"]],
              header=["VENTILATION", "VALUE"],
              title="NET FREE AREA — NOT A TESTED FIGURE",
              align=["l", "l"])

    y -= 10
    notes_block(s, rx, y, rw, "GENERAL NOTES", [
        "FIELD VERIFY all dimensions, roof slope and panel module at the vent "
        "location before fabrication. Dimensions are shop dimensions taken in "
        "the plane of the roof and are OUTSIDE unless noted otherwise.",
        "The manufacturer of record publishes NO dimensions and NO net free "
        "area for this vent. Dimensions shown are code-derived design values. "
        "See open items, sheet SM-2.",
        ...
        "there is no course above to lap under. Locate the vent immediately "
        "below the ridge and carry the rear counterflashing under the ridge "
        "cap. Downslope flange laps OVER the panel below.",
        "DO NOT SOLDER GALVALUME. Joints to be mechanically locked or "
        "riveted and sealed. Isolate from copper and from treated lumber.",
        "Locate the vent within one panel module; do not fasten through a "
    ...
    s.draw_title_block(title_block("SM-1", "5V ROOF VENT — ASSEMBLY", "AS NOTED"))
    s.save()
...
def build_profiles() -> list[sm.Profile]:
    ...
    base = sm.Profile(
        "1 BASE PAN", m,
        segments=[sm.Segment(G.flange_dn, "DNSLOPE FLANGE"),
                  sm.Segment(G.curb_h, "CURB"),
                  sm.Segment(G.opening_l, "OPENING"),
                  sm.Segment(G.curb_h, "CURB"),
                  sm.Segment(G.flange_up, "UPSLOPE FLANGE")],
    ...
    # Hood: one blank. The girth below runs UP THE SLOPE; the two sides fold
    # down from the long edges of the top on a second bend axis, shown as
    # longitudinal bend lines on the flat pattern.
    hood = sm.Profile(
        "2 HOOD", m,
...
BLANK_W = {
    "1 BASE PAN": G.base_w,
    "2 HOOD": G.hood_side_blank,      # top + both sides folded down
    "3 REAR CF": G.base_w,
    "4 SIDE CF": G.hood_l + G.hood_overhang,
}

# Longitudinal bend lines (parallel to the girth), as offsets in from each
# long edge of the blank. Used for parts whose sides fold on a second axis.
...
def sheet_flats(s: Sheet, profiles: list[sm.Profile]):
    ...
    # Flat patterns in two columns, stacked with a computed cursor so the
    # blanks can never collide whatever their proportions.
    SC = 0.085
    COL_X = [1.35, 5.05]          # left edge of each column, paper inches
    DIM_STACK = 0.92              # paper inches of dimension rows below a blank
    TITLE_GAP = 0.34
    ...
    for i, prof in enumerate(profiles, start=1):
        ...
        # sides folded down on a second bend axis
        lb = LONGITUDINAL_BENDS.get(prof.name)
        if lb:
            for zz in (-lb, -W + lb):
                vp.bend_line(0, zz, L, zz)
            vp.text(L * 0.42, -lb + 0.9, "FOLD SIDE DOWN 90°", size=5.2, color=GRAY)
            vp.text(L * 0.42, -W + lb - 2.1, "FOLD SIDE DOWN 90°", size=5.2, color=GRAY)
            vp.dim_v(-lb, 0, 0, offset=-0.34, text=f'{fmt_in(lb)} SIDE')
        prev = 0.0
        ...
        for j, x in enumerate(st):
            b = prof.bends[j]
            vp.text(x + 0.4, -W + 0.8, f"{b.direction} {b.angle:.0f}°",
                    size=5.0, rot=90, color=GRAY)
        # title sits a fixed paper distance below the dimension stack
        vp.view_title(0, -W - (DIM_STACK + TITLE_GAP) / sc,
                      f"FLAT PATTERN — PART {prof.name}",
                      f"GIRTH {fmt_dec(prof.mold_line_girth)} · "
                      f"BLANK {fmt_dec(L)} x {fmt_dec(W)} · QTY {prof.qty}", str(i))
    ...
    y = table(s, rx, y, [96, 92, 30, 66, 66, 126],
              [[p.name, p.material, str(p.qty), fmt_dec(p.mold_line_girth),
                fmt_dec(p.flat_length),
                f"{fmt_dec(p.flat_length)} x {fmt_dec(BLANK_W[p.name])}"]
               for p in profiles],
              header=["PART", "MATERIAL", "QTY", "GIRTH", "FLAT", "BLANK L x W"],
              title="CUT LIST — PER VENT", align=["l", "l", "c", "r", "r", "r"])
    ...
    # ---- forming notes + open items -------------------------------------
    # Open items run the full column width so they stay clear of the title
    # block; forming notes drop below at the narrower width that remains.
    y -= 9
    ...
    notes_block(s, rx, y, 5.25 * IN, "FORMING NOTES", [
        ...
        "DO NOT SOLDER GALVALUME — the Al-Zn coating will not tin and the "
        "joint will corrode.",
        "Protect the coating: clean non-marring dies, no abrasive marking.",
    ], size=5.5, lead=7.4)

    s.draw_title_block(title_block("SM-2", "5V ROOF VENT — FABRICATION", "AS NOTED"))
    s.save()
...
def main():
    ...
    export_flat_patterns(profiles, dxf, widths=BLANK_W,
                         title=f"{P.PROJECT} — 5V ROOF VENT FLAT PATTERNS")
    ...
    print()
    for p in profiles:
        print(f"  {p.name:<12} qty {p.qty}  girth {p.mold_line_girth:7.3f}  "
