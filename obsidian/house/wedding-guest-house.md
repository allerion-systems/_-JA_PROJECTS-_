...
> To-scale floor-plan drawing set (8 basement options, PNG + one-page artifact): [`house/floorplans/`](../../house/floorplans/)
...
| Space | Bed | Guests |
|---|---|---|
| King room (top floor) | King | Kennon + sister — also dress-guard HQ |
| Guest bedroom 1 | Double | Brandon + wife |
...
### Front room — kitchen level (couple A)
- Opening: **60" wide × ~82" tall**. Heavy-duty tension rod mounted 1" below casing top.
- Two 52"×84" blackout panels (104" fabric over 60" = 1.7× fullness), velcro/magnet center closure, Command-hook tie-backs.
- Bed: pullout queen already in the couch — orient so it opens **away** from the curtain line (~7 ft clear when open). Console table = nightstand. Frost film on the front-door fan window.
- Everyone uses the side/back door all weekend — sign on the front door.

### Speakeasy — mid level, "Living Room 3", 12'4.6" × 10'4.8" ($0)
- **Futon pod (couple B):** futon 75×54 against the window wall, head to the corner. Curtain track ~62" off the window wall across 7'4" + ~5'3" return; 22" entry gap. Blackout the window inside; fan at the foot.
- **Stove:** hearth + wood stove stay put; **3-ft no-fabric/no-sleeping radius** at all times.
- **Sectional:** the ~8-ft run opposite the stove, facing the fire.
- **Bar:** owned dresser/table (~30×60) on the door-side wall + 2 stools; tub of ice; lamps only, overheads off. Doorway to hallway/stairs stays clear.
...
> [!WARNING] Layout under review (2026-08-03). Brief evolved twice: first "≤3 walls, no middle lounge" (→ Option E, twin-only), then "**every room must take a queen (60×80)**" — which kills E (5'-4" rooms). Queen-ready sheets: **G — the 8×10 pair**: one 22' wall + one 8' divider (2 walls, 30 lf, ≈ $420), rooms 8×10'-8" and 8×11', queen + window + 32" aisle in BOTH, doors off the existing 3' corridor line, but no basement lounge. **F — user's markup**: two 8×8 queen rooms stacked at the window end (3 walls, 32 lf, ≈ $430), keeps an 11×6 entry lounge, lower room windowless. Note: 8×10 + two 8×8 all at once = 26' of rooms in a 22' room — three queen rooms don't fit. Latest refinements: every queen floats with a **nightstand on each side**; beds head-east with pillows under the hoppers (G/F). **H** drawn per 'bedrooms on other side, doors on opposite side': west rooms + east window gallery — but then **neither bedroom has a window** (both hoppers are on the east wall). Option A below stays the documented build until a pick is made. Couch corrected: **THREE Ashley 4x4 modular sections** — 12' as a row, only lies flat on a 22' wall (today's as-is state, drawn on sheet E); the 11-ft lounges take 2 sections + 1 upstairs. Sheets in `house/floorplans/`.
...
### Yard / sheds
- **7×4 shed = machine bay:** mower nose-in along the 7-ft axis (handle folded), power washer opposite front corner with hose on a wall hook, rail-and-hook kit on the back wall for long tools, top shelf for oil/string/gloves.
...
7. **Day 0:** side-door sign up, dress locked in the king closet, get married.

## Guest-experience playbook

- Text a mini house map before arrival (which door, which floor, where to park). Name card on each room.
- Mornings: 2 showers — lower floors use the entry-level bath, bedroom floor uses upstairs; bride-first upstairs on wedding morning, non-negotiable. Coffee station loaded the night before.
- Nights: speakeasy is the sanctioned afterparty room; white-noise fans in both fabric pods; lamps after 10pm; charger within reach of every bed.
- After: ask every couple for one honest gripe — that's the review system for the Airbnb run.
